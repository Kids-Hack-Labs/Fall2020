...
class IntroScreen():
    ...
    def update(self, delta_ms):
        #Week 12 addition
        self.game_over = self.check_bricks() or self.ball_out
        if self.game_over:
            self.game_won = self.check_bricks()
            return
    ...
    def bound_ball(self):
        ...
        if self.ball.rect.top < 0:
            ...
            self.ball.vel[1] *= -1
        #Week 12 change
        if self.ball.rect.top > self.size[1]:
            self.ball_out = True
        #Week 12 change end
    ...
    #Week 12 function additions
    def check_bricks(self):
        no_bricks_left = True
        for brick in self.bricks:
            if brick.active:
                no_bricks_left = False
                break
        return no_bricks_left
